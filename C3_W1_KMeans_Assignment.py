...
def find_closest_centroids(X, centroids):
    ...
    for i in range(X.shape[0]):
        ...
        for j in range(centroids.shape[0]):
            norm_ij = np.linalg.norm(X[i] - centroids[j])  # Euclidean distance
...
def compute_centroids(X, idx, K):
    ...
    ### START CODE HERE ###
    for k in range(K):   
        # Select all rows in X assigned to centroid k
        points = X[idx == k]

        # Compute the mean of these points and store in centroids[k]
